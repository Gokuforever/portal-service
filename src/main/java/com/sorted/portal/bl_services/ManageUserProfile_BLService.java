...
public class ManageUserProfile_BLService {
	...
	public SEResponse updatePass(@RequestBody SERequest request, HttpServletRequest servletRequest) {
		try {
			...
			String uuid = manageOtp.send(usersBean.getMobile_no(), usersBean.getId(), ProcessType.UPDATE_PASS,
					EntityDetails.USERS, usersBean.getId());
	...
	public SEResponse updatePassVerifyOtp(@RequestBody SERequest request, HttpServletRequest servletRequest) {
		try {
			...
			manageOtp.verify(EntityDetails.USERS, reference_id, otp, entity_id, ProcessType.UPDATE_PASS,
					Defaults.UPDATE_PASS);
